...
//TODO: CREATE METHOD FOR MAKING A NEW VARIABLE
//TODO: KEEP TRACK OF WHICH ARIABLES HAVE BEEN LOCALLY DECLARED ALREADY 
//      AND WHICH VARIABLES ARE BEING DECLARED FOR THE FIRST TIME
//TODO: FIGURE OUT HOW ASSIGNING LABELS TO BLOCKS WILL WORK
//      - it will probably involve keeping a count of already defined blocks
//      - so that you dont define the same block twice (ambiguous)
//      - having different types of block names will make the code more readable
//          - ex: iftrue2:......ifelse2:.....ifend2: while5:...... whileend5...
//      - if you do things this way, you should create the block label BEFORE
//          printing the block's contents:
...
public class VaporConverter extends DepthFirstVisitor {
    ...
    // remember to set calling class in each class declaration 
    ...
    public String newVar() {
        ...
        return var;
        //TODO: should we increment localVarCount as well in this method
    }
    ...
    public void visit(MainClass mc) {
        ...
        //this is probably just for evaluating a 'call' statement (function call)
//        String classTableAlias = "t." + localVarCount++;
//        println(classTableAlias + " = HeapAllocZ(4)");
//        symbolTables.classTables.get(mc_name).vaporAlias.put(mc_name, classTableAlias);
//        println("[" + classTableAlias + "] = vmt_" + mc_name);
        
        NodeListOptional varDeclarations = mc.f14;
        NodeListOptional statements = mc.f15;
//        varDeclarations.accept(this);
    ...
    public void visit(MethodDeclaration md) {
        ...
        /*
            THIS SECTION WILL BE USEFUL FOR ACTUALLY CALLING THESE FUNCTIONS.
            IT ISN'T USEFUL FOR DECLARING THE FUNCTIONS THEMSELVES
        
        //find out calling class/where the class belongs to
        String belongsTo = "";
        if (symbolTables.classTables.get(currentClass).methods.containsKey(methodName)) {
            belongsTo = currentClass;
        }
        
        //if the method isn't in the calling class's method table,
        //keep looking at the calling class's parent's parent's... 
        //method tables until you find the most recent method table
        //with a method of this name
        else if (!symbolTables.classTables.get(currentClass).parentName.isEmpty()) {
            String ancestor = symbolTables.classTables.get(currentClass).getParentName();
            ClassTable ancestorTable = symbolTables.classTables.get(currentClass).getParentTable();
            while (!ancestor.isEmpty() && !ancestorTable.methods.containsKey(methodName)) {
                ancestor = symbolTables.classTables.get(ancestor).getParentName();
                ancestorTable = symbolTables.classTables.get(ancestor);
            }
            if (ancestorTable.methods.containsKey(methodName)) {
                belongsTo = ancestor;
            }
        }
        // at this point, we should have found the correct class to associate with this method call
        if (belongsTo.isEmpty()) {
            error("Error: calling undeclared method");
        }
        */
    }
    ...
    */
    ...
    public void visit(Block b) {
        ...
        b.f1.accept(this);
        indent--;
        // TODO: does every block need a label?
    }
    
    @Override
    public void visit(AssignmentStatement ass) {
        // if the variable has already been declared/ already exists, find out what its alias is
        // else if the variable as not yet been declared, give it a new alias and assign the v
...
//        String expressionResult = prevExpressionResult();
        ...
        println(ifCondition);
        //this is essentially the else block
        indent++;
        ifFalse.accept(this);
        println(gotoEnd);
        indent--;
        //this is essentially the true branch
        println(trueBranch);
        indent++;
        ifTrue.accept(this);
        println(gotoEnd);
    ...
    public void visit(PrintStatement ps) {
        ...
        //store result of expression in a variable
        //PrintIntS(<expression result>)
    }
    ...
    */
    ...
    public void visit(ArrayLength length) {
        ...
        //get location of array produced by evaluating the primary expression
    }
    ...
    public void visit(MessageSend message) {
        ...
        Node callChoice = callingVar.f0.choice;
...
//        println(result + " = " + definedVars.get(name));
//        
//        ClassTable curTable = symbolTables.classTables.get(currentClass);
//        if (curTable.fields.containsKey(name)) {
//            int pos = new ArrayList<String>(curTable.fields.keySet()).indexOf(name);
//            pos += 1; // first 4 bytes are the vmt
//            pos = pos * 4;
//            println(result + " = [this+" + pos + "]");
//        } else {
...
//        }
    ...
    public void visit(BracketExpression bracket) {
        ...
        expression.accept(this);
        //store evaluation of expression in a new variable
    }
